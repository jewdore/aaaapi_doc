---
title: 'chat image mode'
api: 'POST /v1/chat/completions'
description: 'chat image mode'
---

## Overview

This endpoint provides chat image mode functionality.

## Authentication

This endpoint requires authentication using a Bearer token.

<ParamField header="Authorization" type="string" required default="sk-***********">
  Your API key in the format: `YOUR_API_KEY`
</ParamField>

## Request Body

<ParamField body="model" type="string" required default="gemini-2.0-flash-exp-image-generation">
  The model to use for the request. `gemini-2.0-flash-exp-image-generation`
</ParamField>

<ParamField body="messages" type="array" required>
  Array of message objects for the conversation
            <Expandable title="properties">
                <ParamField body="role" type="string" required>
                    * role:user,The role of the message.
                    * role:assistant,The role of the message.
                    * role:system,The role of the message.
                </ParamField>

                <ParamField body="content" type="array" required>

                            <Expandable title="properties">
                                  <ParamField body="type" type="string" required>
                                     text , image_url
                                  </ParamField>

                                    <ParamField body="text" type="string" required>
                                      * type:text,The positive prompt for the generation.
                                      * type:image_url,Optional image reference that guides generation.
                                    </ParamField>
                                    <ParamField body="image_url" type="object" required>
                                      <ParamField body="url" type="string" required>
                                        * type:image_url,The URL of the image to use for the generation.
                                      </ParamField>
                                    </ParamField>
                            </Expandable>

                </ParamField>
            </Expandable>
</ParamField>

<ParamField body="stream" type="boolean"  default="true">
  Whether to stream the response
</ParamField>

## Request Example

<CodeGroup>

```bash cURL
curl -X POST "https://gptproto.com/v1/chat/completions" \
  -H "Authorization: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "model": "gemini-2.0-flash-exp-image-generation",
  "messages": [
    {
      "role": "user",
      "content": "draw you?"
    }
  ],
  "stream": true
}'
```

```python Python
import requests
import json

url = "https://gptproto.com/v1/chat/completions"
headers = {
    "Authorization": "YOUR_API_KEY",
    "Content-Type": "application/json"
}

data = {
  "model": "gemini-2.0-flash-exp-image-generation",
  "messages": [
    {
      "role": "user",
      "content": "draw you?"
    }
  ],
  "stream": true
}

response = requests.post(url, headers=headers, json=data)
result = response.json()
print(json.dumps(result, indent=2))
```

```javascript JavaScript
const url = "https://gptproto.com/v1/chat/completions";
const headers = {
  "Authorization": "YOUR_API_KEY",
  "Content-Type": "application/json"
};

const data = {
  "model": "gemini-2.0-flash-exp-image-generation",
  "messages": [
    {
      "role": "user",
      "content": "draw you?"
    }
  ],
  "stream": true
};

fetch(url, {
  method: "POST",
  headers: headers,
  body: JSON.stringify(data)
})
  .then(response => response.json())
  .then(data => console.log(data))
  .catch(error => console.error("Error:", error));
```

```go Go
package main

import (
    "bytes"
    "encoding/json"
    "fmt"
    "io/ioutil"
    "net/http"
)

func main() {
    url := "https://gptproto.com/v1/chat/completions"

    payload := []byte(`{
"model": "gemini-2.0-flash-exp-image-generation",
"messages": [
{
"role": "user",
"content": "draw you?"
}
],
"stream": true
}`)

    req, _ := http.NewRequest("POST", url, bytes.NewBuffer(payload))
    req.Header.Set("Authorization", "YOUR_API_KEY")
    req.Header.Set("Content-Type", "application/json")

    client := &http.Client{}
    resp, err := client.Do(req)
    if err != nil {
        panic(err)
    }
    defer resp.Body.Close()

    body, _ := ioutil.ReadAll(resp.Body)
    fmt.Println(string(body))
}
```

</CodeGroup>

## Response

<ResponseField name="Success" type="200">
  Successful response

```json
{
  "status": "success"
}
```
</ResponseField>

## Error Responses

<ResponseExample>

```json 401 - Invalid signature
{
  "error": {
    "message": "Invalid signature",
    "type": "401"
  }
}
```

```json 403 - Invalid Token
{
  "error": {
    "message": "Invalid Token",
    "type": "403"
  }
}
```

```json 403 - Insufficient balance
{
  "error": {
    "message": "Insufficient balance",
    "type": "403"
  }
}
```

```json 500 - Internal server error
{
  "error": {
    "message": "Internal server error",
    "type": "500"
  }
}
```

```json 503 - Content policy violation
{
  "error": {
    "message": "Input may not meet the guidelines. Please adjust and try again.",
    "type": "503"
  }
}
```

</ResponseExample>

