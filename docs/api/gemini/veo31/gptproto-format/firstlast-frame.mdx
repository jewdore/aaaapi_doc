---
title: "First&last frame"
api: "POST /v1/veo/videos"
description: "First&last frame"
---

## Overview

This endpoint provides first&last frame functionality.

## Authentication

This endpoint requires authentication using a Bearer token.

<ParamField header="Authorization" default="sk-***********" type="string" required>
  Your API key in the format: `Bearer YOUR_API_KEY`
</ParamField>

## Request Body

<ParamField body="prompt" default="A beautiful woman is dancing in a European and American style" type="string" required>
  Prompt parameter
</ParamField>

<ParamField body="model" default="veo3.1-pro" type="string" required>
  The model to use for the request
</ParamField>

<ParamField body="upsample" default="true" type="boolean">
  Upsample parameter
</ParamField>

<ParamField body="reference" default="false" type="boolean">
  Reference parameter
</ParamField>

<ParamField body="aspect_ratio" default="16:9" type="string">
  Aspect Ratio parameter
</ParamField>

<ParamField body="images" default='["https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170918_yyy1.jpeg", "https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170932_yyy2.jpeg"]' type="array">
  Images parameter
</ParamField>

## Request Example

<CodeGroup>

```bash cURL
curl -X POST "https://gptproto.com/v1/veo/videos" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "prompt": "A beautiful woman is dancing in a European and American style",
  "model": "veo3.1",
  "upsample": true,
  "reference": false,
  "aspect_ratio": "16:9",
  "images": [
    "https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170918_yyy1.jpeg",
    "https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170932_yyy2.jpeg"
  ]
}'
```


```python Python
import requests
import json

url = "https://gptproto.com/v1/veo/videos"
headers = {
    "Authorization": "Bearer YOUR_API_KEY",
    "Content-Type": "application/json"
}

data = {
  "prompt": "A beautiful woman is dancing in a European and American style",
  "model": "veo3.1",
  "upsample": true,
  "reference": false,
  "aspect_ratio": "16:9",
  "images": [
    "https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170918_yyy1.jpeg",
    "https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170932_yyy2.jpeg"
  ]
}

response = requests.post(url, headers=headers, json=data)
result = response.json()
print(json.dumps(result, indent=2))
```


```javascript JavaScript
const url = "https://gptproto.com/v1/veo/videos";
const headers = {
  "Authorization": "Bearer YOUR_API_KEY",
  "Content-Type": "application/json"
};

const data = {
  "prompt": "A beautiful woman is dancing in a European and American style",
  "model": "veo3.1",
  "upsample": true,
  "reference": false,
  "aspect_ratio": "16:9",
  "images": [
    "https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170918_yyy1.jpeg",
    "https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170932_yyy2.jpeg"
  ]
};

fetch(url, {
  method: "POST",
  headers: headers,
  body: JSON.stringify(data)
})
  .then(response => response.json())
  .then(data => console.log(data))
  .catch(error => console.error("Error:", error));
```


```go Go
package main

import (
    "bytes"
    "encoding/json"
    "fmt"
    "io/ioutil"
    "net/http"
)

func main() {
    url := "https://gptproto.com/v1/veo/videos"

    payload := []byte(`{
"prompt": "A beautiful woman is dancing in a European and American style",
"model": "veo3.1",
"upsample": true,
"reference": false,
"aspect_ratio": "16:9",
"images": [
"https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170918_yyy1.jpeg",
"https://heyoo.oss-ap-southeast-1.aliyuncs.com/20251017_170932_yyy2.jpeg"
]
}`)

    req, _ := http.NewRequest("POST", url, bytes.NewBuffer(payload))
    req.Header.Set("Authorization", "Bearer YOUR_API_KEY")
    req.Header.Set("Content-Type", "application/json")

    client := &http.Client{}
    resp, err := client.Do(req)
    if err != nil {
        panic(err)
    }
    defer resp.Body.Close()

    body, _ := ioutil.ReadAll(resp.Body)
    fmt.Println(string(body))
}
```

</CodeGroup>

## Response

<ResponseField name="Success" type="200">
  Successful response

  ```json
  {
    "status": "success"
  }
  ```
</ResponseField>

## Error Responses

<ResponseExample>

```json 401 - Invalid signature
{
  "error": {
    "message": "Invalid signature",
    "type": "401"
  }
}
```


```json 403 - Invalid Token
{
  "error": {
    "message": "Invalid Token",
    "type": "403"
  }
}
```


```json 403 - Insufficient balance
{
  "error": {
    "message": "Insufficient balance",
    "type": "403"
  }
}
```


```json 500 - Internal server error
{
  "error": {
    "message": "Internal server error",
    "type": "500"
  }
}
```


```json 503 - Content policy violation
{
  "error": {
    "message": "Input may not meet the guidelines. Please adjust and try again.",
    "type": "503"
  }
}
```

</ResponseExample>