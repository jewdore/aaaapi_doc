...
| `Content-Type` (Header) | string | ✅ Yes | `application/json` | Must be set to `application/json` |
| `model` | string | ✅ Yes | `gpt-4.1` | The model to use for the request. Must be a vision-enabled model. |
| `input` | array | ✅ Yes | - | Array of message objects with role and content |
| `stream` | boolean | ❌ No | `false` | Whether to stream the response |

### Input Array Structure

Each message object in the `input` array should have the following structure:
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `type` | string | ✅ Yes | Type of content. Can be: `input_text` or `input_image` |
| `text` | string | ✅ Yes (if type is `input_text`) | Text prompt content |
| `image_url` | string | ✅ Yes (if type is `input_image`) | Image URL or base64 encoded image (`data:image/jpeg;base64,{base64_string}`) |
